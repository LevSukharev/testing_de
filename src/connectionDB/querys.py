...
def insert_contact_details_query(user: User, user_id: int):
    insert_contact_details_query = """
            ...
            values (%s, %s, %s)
            """
    contact_details_params = (user_id, user.phone, user.cell)
...
def insert_media_data_query(user: User, user_id: int):
    insert_media_data_query = """
            ...
            values (%s, %s)
            """
    media_data_params = (user_id, user.picture.thumbnail)
...
def select_all(
    valid_password: bool = False, valid_email: bool = False, limit: int | str = " "
):
